...
function spawnBosses() {
  ...
  const saur  = _anchorNear('baradDur');
  ...
  state.entities.push(makeBossSauron(saur.x, saur.y));
}

// Listen for boss death to emit a canonical event & reward renown.
on('enemyKilled', ({ entity }) => {
  if (!entity || !entity.isBoss) return;
  const reward = entity.type === 'bossSauron' ? 5000
    : entity.type === 'bossWK' ? 2000
    : entity.type === 'bossUruk' ? 700 : 400;
  state.renown += reward;
